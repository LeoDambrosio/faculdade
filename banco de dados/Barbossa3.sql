...
-- Query para tabelas de preço ativas com produtos pesados e pedidos para parceiro específico
SELECT DISTINCT
    tp.idtabelapreco AS "ID Tabela",
    tp.descricao AS "Descrição Tabela",
    tp.codigo AS "Código",
    tp.iniciovigencia AS "Início Vigência",
    tp.fimvigencia AS "Fim Vigência",
    p.idproduto AS "ID Produto",
    p.descricao AS "Produto",
    p.peso AS "Peso",
    pp.preco AS "Preço",
    par.idparceiro AS "ID Parceiro",
    par.nome AS "Nome Parceiro"
FROM
    tabelapreco tp
JOIN
    precoproduto pp ON tp.idtabelapreco = pp.idtabelapreco
JOIN
    produto p ON pp.idproduto = p.idproduto
JOIN
    pedido ped ON ped.idtabelapreco = tp.idtabelapreco
JOIN
    pedidoproduto pped ON (ped.idpedido = pped.idpedido AND pped.idproduto = p.idproduto)
JOIN
    parceiro par ON ped.idparceiro = par.idparceiro
WHERE
    CURRENT_DATE BETWEEN tp.iniciovigencia AND tp.fimvigencia
    AND p.peso > 3.0  -- Substitua 3.0 pelo valor X desejado
    AND par.idparceiro = 1  -- Substitua 1 pelo ID do parceiro X desejado
ORDER BY
    tp.descricao, p.peso DESC;
